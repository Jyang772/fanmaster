...
int fansLevel = AUTO;
int sleepTime = 120, criticalTemp = 105, safeTemp = 58, autoSpeedValue = 7;
gint timeout;
...
int main (int argc, char *argv[]){
    ...
    gtk_builder_add_from_file (builder, "gtk_gui.glade"/*BUILDER_XML_FILE*/, NULL);
    ...
    timeout_manual = g_timeout_add_seconds(1,status,NULL);
...
int fans(){
    ...
    fgets(fanRPM,sizeof(fanRPM),fp);

    // writing CPU temperature, and time when it's checked
    sprintf(message,"Temperature: %dC, Fan: %dRPM Checked at %s",temp,atoi(fanRPM),tmpString);
